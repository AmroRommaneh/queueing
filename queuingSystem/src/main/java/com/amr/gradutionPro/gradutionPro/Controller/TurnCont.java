...
@Controller
@RequestMapping("api/Gradution/Turn")
public class TurnCont {
    ...
    public ResponseEntity<String> creatTurn(@RequestBody TurnDto turnDto) {
        ...
        if (GradutionProApplication.turns.size() <= 100 && clientExist == 0) {
            ...
            User user =userRep.findById(turnDto.getClientId());


            String u = "Dear "+ user.getName() +"your turn is reserved \n, turn detalis:\n  starts after " + turn.getEstimatedTimeToStart()+"miniuts .\n" +
                     "Picked at  " + nowTime.format(DateTimeFormatter.ofPattern("HH:mm:ss")) +
                    " \nyour turn will be done at window " + GradutionProApplication.turns.get(GradutionProApplication.turns.size() - 1).getWindowSelected() +
                    "\n and the whole process will take" + turn.getEstimatedTime() +"miniuts .\n" + " Service Time" + serviceTime;
...
System.out.println(id.getClientId());
        ...
        if (x == null) {
            return new ResponseEntity<>("No turn Found", HttpStatus.NOT_FOUND);
